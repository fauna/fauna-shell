// @ts-check

import { expect } from "chai";
import sinon from "sinon";

import { run } from "../../../src/cli.mjs";
import { setupTestContainer as setupContainer } from "../../../src/config/setup-test-container.mjs";
import { colorize, Format } from "../../../src/lib/formatting/colorize.mjs";
import { mockAccessKeysFile } from "../../helpers.mjs";

const createExportStub = (opts) => ({
  id: "test-export-id",
  state: "Pending",
  database: "us-std/example",
  format: "simple",
  destination: {
    s3: {
      path: "/test/key",
      bucket: "test-bucket",
    },
  },
  created_at: "2025-01-02T22:59:51",
  ...opts,
});

describe("database create s3", () => {
  let container, stderr, stdout, makeAccountRequest;

  beforeEach(() => {
    container = setupContainer();
    stderr = container.resolve("stderrStream");
    stdout = container.resolve("stdoutStream");
    makeAccountRequest = container.resolve("makeAccountRequest");

    mockAccessKeysFile({ fs: container.resolve("fs") });
  });

  it("creates an export", async () => {
    const database = "us-std/example";
    const bucket = "test-bucket";
    const path = "/test/key";
    const stubbedResponse = createExportStub({
      database,
      destination: {
        s3: {
          path,
          bucket,
        },
      },
      format: "simple",
    });
    makeAccountRequest.resolves(stubbedResponse);

    await run(
      `export create s3 --database '${database}' --bucket '${bucket}' --path '${path}'`,
      container,
    );
    await stdout.waitForWritten();

    expect(stdout.getWritten()).to.equal("test-export-id\n");
    expect(makeAccountRequest).to.have.been.calledWith({
      method: "POST",
      prefix: "/v2",
      path: "/exports",
      body: JSON.stringify({
        database,
        destination: {
          s3: {
            bucket,
            path,
          },
        },
        format: "simple",
      }),
      secret: sinon.match.string,
    });
  });

  it("outputs the full response with --json", async () => {
    const database = "us-std/example";
    const bucket = "test-bucket";
    const path = "/test/key";
    const stubbedResponse = createExportStub({
      database,
      destination: {
        s3: {
          path,
          bucket,
        },
      },
      format: "simple",
    });
    makeAccountRequest.resolves(stubbedResponse);

    await run(
      `export create s3 --database '${database}' --bucket '${bucket}' --path '${path}' --json`,
      container,
    );
    await stdout.waitForWritten();

    expect(stdout.getWritten()).to.equal(
      `${colorize(stubbedResponse, { format: Format.JSON })}\n`,
    );
  });

  it("standardizes the database region", async () => {
    makeAccountRequest.resolves(
      createExportStub({ database: "us-std/example" }),
    );
    await run(
      `export create s3 --database 'us/example' --bucket 'test-bucket' --path 'test/key'`,
      container,
    );
    expect(makeAccountRequest).to.have.been.calledWith(
      sinon.match({
        body: sinon.match('"database":"us-std/example"'),
      }),
    );
  });

  it("passes the format to the account api", async () => {
    makeAccountRequest.resolves(createExportStub({ format: "tagged" }));
    await run(
      `export create s3 --database 'us-std/example' --bucket 'test-bucket' --path 'test/key' --format 'tagged'`,
      container,
    );
    expect(makeAccountRequest).to.have.been.calledWith(
      sinon.match({
        body: JSON.stringify({
          database: "us-std/example",
          destination: {
            s3: {
              bucket: "test-bucket",
              path: "test/key",
            },
          },
          format: "tagged",
        }),
      }),
    );
  });

  it("should fail if --local is used", async () => {
    try {
      await run(
        "export create s3 --bucket test-bucket --path test/key --local",
        container,
      );
    } catch {}

    await stderr.waitForWritten();
    expect(stderr.getWritten()).to.contain(
      "Exports do not support --local and the Fauna docker container.",
    );
  });

  it("should fail if --secret is provided", async () => {
    try {
      await run(
        "export create s3 --bucket test-bucket --path test/key --secret=some-test-secret",
        container,
      );
    } catch {}

    await stderr.waitForWritten();
    expect(stderr.getWritten()).to.contain(
      "Exports are not supported with --secret. Use --database instead.",
    );
  });

  it("should fail if --database is not provided", async () => {
    try {
      await run(
        "export create s3 --bucket test-bucket --path test/key",
        container,
      );
    } catch {}

    await stderr.waitForWritten();
    expect(stderr.getWritten()).to.contain(
      "--database is required to create an export.",
    );
  });
});
