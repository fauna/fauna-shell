...
describe('schema diff', function() {
  const colorDiffString = "\u001b[1;34m* Removing collection `Todo`\u001b[0m from main.fsl:8:1:\n\n\u001b[31m  - collection Todo {\u001b[0m\n\u001b[31m  -   history_days 0\u001b[0m\n\u001b[31m  - }\u001b[0m\n\n"
  const noColorDiffString = "* Removing collection `Todo` from main.fsl:8:1:\n\n  - collection Todo {\n  -   history_days 0\n  - }\n\n"
  let container,
    logger
  ...
  it('can display the diff between local and remote schema', async function() {
    ...
    expect(fetch).to.have.been.calledWith("https://db.fauna.com/schema/1/validate?force=true&color=ansii&staged=false", { ... commonFetchParams, method: 'POST' })
  ...
  it('can display the diff between local and staged remote schema', async function() {
    ...
    expect(fetch).to.have.been.calledWith("https://db.fauna.com/schema/1/validate?force=true&color=ansii&staged=true", { ... commonFetchParams, method: 'POST' })
    expect(logger.stdout).to.have.been.calledWith(colorDiffString)
    expect(logger.stderr).to.not.have.been.called
  ...
  it('can display the diff without color (terminal escape codes)', async function() {
    ...
    expect(fetch).to.have.been.calledWith("https://db.fauna.com/schema/1/validate?force=true&staged=false", { ... commonFetchParams, method: 'POST' })
