...
describe('schema pull', function() {
  ...
  it('can pull schema', async function() {
    ...
    fetch.onCall(2).resolves(f({
      content: '[{"name":"main.fsl","content":"collection Main {\\n  name: String\\n  index byName {\\n    terms [.name]\\n  }\\n}\\n\"}]'
    }))
    fetch.onCall(3).resolves(f({
      content: '[{"name":"second.fsl","content":"collection Second {\\n  name: String\\n  index byName {\\n    terms [.name]\\n  }\\n}\\n\"}]'
    }))
    fetch.onCall(4).resolves(f({
      content: '[{"name":"third.fsl","content":"collection Third {\\n  name: String\\n  index byName {\\n    terms [.name]\\n  }\\n}\\n\"}]'
    }))
    ...
    } catch (e) {console.error(logger.stderr.args.join('\n'))}
    ...
    expect(fetch).to.have.been
      .calledWith('https://db.fauna.com/schema/1/files', commonFetchParams)
    // the version param in the URL is important - we use it for optimistic locking
    expect(fetch).to.have.been
      .calledWith('https://db.fauna.com/schema/1/staged/status?version=194838274939473', commonFetchParams)
    expect(fetch).to.have.been
      .calledWith('https://db.fauna.com/schema/1/files/main.fsl', commonFetchParams)
    expect(fetch).to.have.been
      .calledWith('https://db.fauna.com/schema/1/files/second.fsl', commonFetchParams)
    expect(fetch).to.have.been
      .calledWith('https://db.fauna.com/schema/1/files/third.fsl', commonFetchParams)
    ...
    expect(logger.stdout).to.have.been.calledWith("Pull makes the following changes:")
