...
describe('schema status', function() {
  ...
  it('fetches the current status', async function() {
    ...
    expect(fetch).to.have.been.calledWith("https://db.fauna.com/schema/1/staged/status?diff=true&color=ansii", commonFetchParams)
  })
  ...
  it('can fetch status without embedded colors (terminal escape codes)', async function() {
    ...
    expect(fetch).to.have.been.calledWith("https://db.fauna.com/schema/1/staged/status?diff=true", commonFetchParams)
  })
})
